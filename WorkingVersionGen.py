...
def print_flashing_text(text, red=False):
    if red:
        print("\033[41m\033[37m" + text + "\033[0m", end="\r")
    else:
        print("\033[32m" + text + "\033[0m", end="\r")
    time.sleep(0.5)
    print("\033[0m", end="\r")
    time.sleep(0.5)
    logging.info(f'Flashing text printed: {text}')
...
def main(num_images, prompt, nprompt, num_steps, width, height, lora_model, show, pipe, compel):
    ...
    # Logic for the handling the prompt token length increase and === tensor shape
    compel = Compel(tokenizer=[pipe.tokenizer, pipe.tokenizer_2] , text_encoder=[pipe.text_encoder, pipe.text_encoder_2], returned_embeddings_type=ReturnedEmbeddingsType.PENULTIMATE_HIDDEN_STATES_NON_NORMALIZED, requires_pooled=[False, True]) # try trutru?
